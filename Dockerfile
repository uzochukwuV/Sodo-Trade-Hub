...
# ── Generate SQL migration files from the TypeScript schema ──────────────────
# drizzle-kit generate only reads TS schema files — it never connects to the DB.
# The DATABASE_URL check in drizzle.config.ts just needs a non-empty string.
RUN DATABASE_URL=postgres://build-placeholder \
...
# SQL migration files generated by drizzle-kit (applied at container startup)
